...
🟢 `[status: active]` `[lane: docs]` `[type: index]` `[pin: hard-ref]`

Repo orientation, full file inventory with statuses, archive manifest, and a
task-to-doc navigation index live in `docs/cowork/` (00–03). Archived docs live
in `docs/archive/` and are tracked in `docs/cowork/02_repo_archive_manifest.md`
plus `docs/64_repo_cleanup_archive_index.md`.

> **2026-06-09:** all implemented planning artifacts are archived under
> `docs/archive/2026-06-implemented-plans/`; planning-doc paths in this map
> already point there. The live doc surface is defined by `docs/README.md`.
...
`docs/archive/2026-06-implemented-plans/46_forge_ui_stack_research.md`,
`docs/47_forge_ui_phase_tracker.md`, and
`docs/48_forge_ui_component_inventory.md`. The Tailwind/shadcn adoption gate
result lives in `docs/archive/2026-06-implemented-plans/50_forge_ui_tailwind_shadcn_gate.md`. Remaining global CSS
...
`docs/archive/2026-06-implemented-plans/49_list_controls_focus_layout_backlog.md`; it is the source of truth for
the shared search/sort/filter row-control pattern across Maker, Card Browser,
Decks, Collections, Binders, and Lists.
The Deck Builder export/preview repair lane lives in
`docs/archive/2026-06-implemented-plans/69_deck_builder_export_preview_repair_plan.md`; use it before changing
deck export context, deck variants in export payloads, Focused Card Preview
routing, deck add/swap flows, References official-card view controls, or shared
card-list icon/selection behavior.
The trust-and-standards usability lane lives in
`docs/archive/2026-06-implemented-plans/70_usability_trust_standards_roadmap.md` and
`docs/archive/2026-06-implemented-plans/71_usability_trust_standards_backlog.md`; use it before changing global
dropdown styling, shared sort controls, loading/empty-state copy, Home/startup
behavior, side-rail ordering, or Work Mode customization.
...
`docs/archive/2026-06-implemented-plans/52_editor_review_issue_catalog.md`,
`docs/archive/2026-06-implemented-plans/53_editor_review_priority_matrix.md`, and
`docs/archive/2026-06-implemented-plans/54_editor_review_phase_tracker.md`. Use these docs before changing laptop
...
in `docs/archive/2026-06-implemented-plans/external-research-legacy/` (relocated legacy copy) and has been distilled into
`docs/archive/2026-06-implemented-plans/58_external_research_next_version_backlog.md` and
`docs/archive/2026-06-implemented-plans/59_next_version_phase_roadmap.md`. These are planning inputs only until
...
`docs/archive/2026-06-implemented-plans/superpowers-plans/2026-06-08-shared-delivery-modes.md`; tooling prep and
storage/path decisions live in `docs/archive/2026-06-implemented-plans/60_desktop_delivery_tooling_prep.md`. The
final active roadmap now lives in
`docs/archive/2026-06-implemented-plans/62_shared_desktop_delivery_final_roadmap.md`; route extraction is tracked
in `docs/61_runtime_service_route_inventory.md`; phase status lives in
`docs/archive/2026-06-implemented-plans/63_shared_desktop_delivery_phase_tracker.md`; cleanup/archive rules live
...
`docs/archive/2026-06-implemented-plans/66_desktop_cutover_checklist.md`. The completed external research packet
is preserved at
`docs/archive/2026-06-implemented-plans/superpowers-plans/homebrew_forge_desktop_delivery_research_packet/`. The
...
aids, such as the archived
`docs/archive/2026-06-packet-era/20_commander_token_print_manifest.md`
(print aids delivered; future print-sheet work lives in
`docs/archive/2026-06-implemented-plans/37_print_export_research_and_roadmap.md`). These manifests are
...
`docs/archive/2026-06-implemented-plans/21_create_import_export_overlay_plan.md` and
`docs/archive/2026-06-implemented-plans/22_create_import_export_priority_matrix.md`. Full-app import/export
polish, variant-aware CSV intake, deck/set/collection/library/reference import
phases, and project package staging live in
`docs/archive/2026-06-implemented-plans/27_import_export_full_app_polish_plan.md`. Use those docs before
...
`docs/archive/2026-06-implemented-plans/37_print_export_research_and_roadmap.md`. Use the shared Forge print
...
`docs/archive/2026-06-implemented-plans/28_collection_deck_workspace_polish_plan.md` and
`docs/archive/2026-06-implemented-plans/29_collection_deck_workspace_priority_matrix.md`. Use those docs before
...
`docs/archive/2026-06-implemented-plans/57_collections_manager_v1_5_plan.md`; use it before changing collection
...
`docs/archive/2026-06-implemented-plans/32_card_dashboard_research_plan.md` and
`docs/archive/2026-06-implemented-plans/33_card_dashboard_priority_matrix.md`. Use those docs before adding
...
Work Modes planning lives in `docs/archive/2026-06-implemented-plans/34_work_modes_research.md`,
`docs/35_work_modes_personas_requirements.md`, and
`docs/archive/2026-06-implemented-plans/36_work_modes_priority_matrix.md`. V1 Work Modes are Full Studio, Card
...
`docs/frame-taxonomy.md`, `docs/archive/2026-06-implemented-plans/frame-support-roadmap.md`,
...
Backend source asset cataloging lives in `assets/catalog/` and
`docs/72_backend_asset_catalog.md`; run `forge assets catalog --write`, then
`python3 scripts/mcp/asset-catalog/build_index.py --repo-root .` before future
frame/symbol promotion work so mtg-vectors, GenevensiS, and Basic/Full M15
assets are discovered through the `homebrew-forge-assets` MCP instead of ad hoc
folder searches.
Set symbols are now set-owned metadata on `sets/*/sets.csv` via
`set_symbol_asset_id`, `set_symbol_path`, and `set_symbol_url`; renderer output
uses the set symbol in the type line and prefers rarity-specific sibling files
such as `C.svg`, `U.svg`, `R.svg`, `M.svg`, and `T.svg` when present.
...
assets/
  ...
  catalog/               # Backend source-asset catalog config plus generated searchable indexes for local frame/symbol libraries
...
scripts/
  ...
  codex/
    ...
    signs-of-assassins/
      upsert-odyssey-ability-packet.ts
                          # Rerunnable SOA Odyssey ability-card packet upsert: Hero Strike normalization, SOA-003..SOA-014 cards/art/variants, and V8 maybeboard rows
  ...
  mcp/
    asset-catalog/        # Local stdio MCP server plus SQLite/FTS builder for searchable frame, symbol, stamp, icon, watermark, vector, chroma, and relationship lookup
  macos/
...
sets/
  ...
  SOA/                   # Signs of Assassins authored private-playtest cards, including Odyssey ability tests and Assassin deck-building experiments
decks/
  ...
  signs-of-assassins/    # Altaïr Commander deck family with eight variants and collection/recommendation-backed official, proxy, and SOA maybeboard rows
...
packages/
  forge/                 # CLI and core implementation
    src/
      ...
      trash/             # Recoverable local trash manifests and restore payloads for top-level Forge objects
  ...
  editor/                # Vite/React local editor for selecting cards, editing fields, and previewing frames
    src/
      ...
      components/        # Toolbar/menu shell, rail, workspace views, overlays, image lightbox, Cockatrice-style card browser, collection import/price/transfer panels, deck builder, management lists, card list, preview, inspector, tabs, and field controls
        ...
        forge-ui/         # Dependency-free Forge UI primitives and shared controls used during the CSS/component-first migration
        overlays/         # Reusable overlays for official-card destination, destructive confirmation, deck, collection, and reference workflows
        print/            # File > Print overlay, print queue, and calibration settings surface
  ...
  runtime-service/       # Embeddable local HTTP runtime for shared web/desktop delivery
    src/
      createRuntimeServer.ts
                          # Starts a 127.0.0.1 runtime with selected-port fallback, health/version/trash routes, and optional built-editor static serving
...
- Deck metadata upgrade planning lives in `docs/archive/2026-06-implemented-plans/30_deck_metadata_format_upgrade_plan.md`; deck stats research lives in `docs/archive/2026-06-implemented-plans/31_deck_stats_research_notes.md`; reusable editor suggestions for formats, play styles, and Commander brackets live in `packages/editor/src/domain/deckTaxonomy.ts`. Deck cover presentation lives in `packages/editor/src/components/decks/DeckCoverBadge.tsx`, and warning-only Commander/format checks live in `packages/editor/src/domain/deckLegality.ts`.
...
- Official-card quick actions in References and the Card Browser open `OfficialCardDestinationOverlay` before mutating data. Deck adds require a deck variant; collection adds require a destination collection/new collection; set copies require a target set and copy status.
- Collection-backed official deck entries expose a print replacement path that keeps authored-card variants separate from official Scryfall print identity. Repeated official adds merge only within the selected deck variant.
- Top-level deletes use `packages/forge/src/trash/` and the Trash workspace. Use the verbs "Move to Trash," "Restore," and "Permanently Delete" for recoverable object deletion; use "Remove from deck," "Remove from collection," and "Move out of project" for membership changes.
- Trash payloads are stored under ignored `.homebrew-forge/trash/items/<trashId>/` directories with 30-day recoverability. Vite and runtime-service expose `GET /api/trash`, `POST /api/trash/move`, `POST /api/trash/restore`, and `POST /api/trash/empty`.
- Print queue lives behind File > Print with authored-card, deck-entry, collection-row, and official-print queue items. 4x6 paper forces one card per page, and the calibration preset emits a wireframe proof with explicit 100% actual-size print guidance while preserving the 63mm x 88mm slot.
- Use the shared icon-based `ViewModeControl` for grid/list/single controls across Sets, Decks, Collections, Lists, Binders, Cards, and official-card surfaces. Decks may add board/candidates/roles/mana as deck-specific modes.
...
- Keep card-surface tool modes in `packages/editor/src/components/CardPreview.tsx`: Zoom opens the larger preview on card click, Artwork owns artwork move/crop, Text owns direct edits to name/type/rules/flavor/stat fields, and Layout V1 owns rules-text padding handles plus text-zone selection. Broader name/type/text-box positioning is future work, not part of the shipped V1. UX consistency audit and tracker live in `docs/archive/2026-06-implemented-plans/55_ux_consistency_conventions_audit.md` and `docs/archive/2026-06-implemented-plans/56_ux_consistency_phase_tracker.md`.
