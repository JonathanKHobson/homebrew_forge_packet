...
| 2A | P1 | Card create overlay | Cards plus opens overlay; create draft only after confirm; opens saved draft in Cards workspace. | Bulk card import and live preview tuning. | New card is not created until `Create Draft`; saved card opens for editing. |
...
| 2E | P1 | Library create overlay | Library plus opens overlay with upload/URL/metadata UI and staged unsupported states. | Bulk asset import persistence. | Overlay is consistent and does not claim unsupported imports completed. |
| 3A | P1 | Import hub IA | File > Import shows Cards/Decks/Sets/Projects/Library/References choices. | Real new importers beyond existing set/card flow. | Existing active-set import remains usable. |
...
| 3F | P3 | Library import | Multi-file, URL, or CSV metadata asset import. | Automatic art licensing decisions. | Assets appear in Library with metadata or blank review state. |
...
| 4E | P3 | Project export | Export project package with sets, decks, cards, library assets, and references. | Public sharing/gallery. | Project package can be reimported in a later phase. |
| 4F | P3 | Library export | Export selected or bulk assets with metadata. | Asset license automation. | Files and metadata export together. |
| 4G | P3 | Reference export | Export bundled plus local custom reference data after the Reference model stabilizes. | New Reference storage locations. | Export uses `reference/custom/references.json` plus bundled catalog source labels. |
...
2. Replace the Cards plus button with the Card creation overlay.
3. Preserve the existing card save path and open the saved draft after confirm.
4. Do not change Decks/Sets/Projects/Library plus buttons until Cards proves
   the pattern.

Reason: Cards is the clearest UX problem because it currently creates an object
