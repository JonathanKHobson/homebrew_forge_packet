...
import type { CollectionImportMode, CollectionImportSummary, CollectionSourcePreset } from '../domain/editorTypes.js';
...
export function CollectionImportPanel({ defaultCollectionId = 'current-cards', onImported, onStatus }: CollectionImportPanelProps) {
  ...
  async function runImport(dryRun: boolean) {
    ...
    try {
      ...
      onStatus(`${dryRun ? 'Dry-run analyzed' : 'Imported'} ${result.summary.importedRows} rows. ${result.summary.reviewRows} need review.`);
      if (!dryRun) {
    ...
    <div className="collection-import-panel">
      <div className="collection-import-grid">
        <Field label="CSV file">
          <input type="file" accept=".csv,text/csv,text/plain" onChange={(event) => void handleFile(event.target.files?.[0])} />
        </Field>
        <Field label="Source">
      ...
      <Field label="CSV preview">
        <textarea value={content} rows={6} placeholder="Paste scanner CSV here, or choose a file above." onChange={(event) => setContent(event.target.value)} />
      </Field>
